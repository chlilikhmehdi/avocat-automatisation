...
export default function Sidebar({ currentUser, onLogout }) {
  ...
  const navItems = [
    ...(role === 'ADMIN'
      ? [{ path: '/users', icon: '👥', label: t.users }]
      : []),
    ...(['ADMIN', 'LAWYER'].includes(role)
      ? [{ path: '/cases', icon: '📁', label: t.cases }]
      : []),
    { path: '/audiences', icon: '📅', label: t.audiences, disabled: true },
    { path: '/documents', icon: '📄', label: t.documents, disabled: true },
  ];

  return (
    <div className="sidebar" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      <div className="sidebar-logo">
        <div className="sidebar-logo-dot" />MiZan
      </div>
      ...
      <div className="sidebar-footer">
        <div className="sidebar-user" onClick={onLogout} title={t.logout}>
          <UserAvatar name={currentUser?.nom} role={role} />
          <div>
